...
namespace rabitqlib {
  ...
  inline void assert_integral() {
    static_assert(std::is_integral_v<T>, "Template type T must be an integral type (int, long, unsigned, etc.).");
  }
  ...
  inline bool is_powerof2(size_t n) {
    return n > 0 && (n & (n - 1)) == 0;
  }
  ...
  float distance_ratio(const T *data, const T *query, const TP *gt, const TP *ann_results, size_t k, size_t dim,
