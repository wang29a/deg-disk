...
namespace rabitqlib {
  ...
  namespace rotator_impl {
    ...
    class MatrixRotator : public Rotator<T> {
     ...
     public:
      explicit MatrixRotator(size_t dim, size_t padded_dim) : Rotator<T>(dim, padded_dim), rand_mat_(dim, padded_dim) {
        RowMajorMatrix<T> rand = random_gaussian_matrix<T>(padded_dim, padded_dim);
        Eigen::HouseholderQR<RowMajorMatrix<T>> qr(rand);
        RowMajorMatrix<T> q_inv = qr.householderQ().transpose();  // inverse of orthogonal mat is its inverse
    ...
    static inline void flip_sign(const uint8_t *flip, float *data, size_t dim) {
      ...
      static_assert(kFloatsPerChunk % 16 == 0, "floats_per_chunk must be divisible by AVX512 register width");
    ...
    class FhtKacRotator : public Rotator<float> {
     ...
     public:
      ...
      void load(std::ifstream &input) override {
        input.read(reinterpret_cast<char *>(flip_.data()), static_cast<long>(sizeof(uint8_t) * flip_.size()));
      }

      void save(std::ofstream &output) const override {
        output.write(reinterpret_cast<const char *>(flip_.data()), static_cast<long>(sizeof(uint8_t) * flip_.size()));
      }
  ...
  template<typename T>
  Rotator<T> *choose_rotator(size_t dim, RotatorType type = RotatorType::FhtKacRotator, size_t padded_dim = 0) {
    if (padded_dim == 0) {
      padded_dim = rotator_impl::padding_requirement(dim, type);
      if (padded_dim != dim) {
        std::cerr << "vectors are padded to " << padded_dim << " dimensions for aligned computation\n";
